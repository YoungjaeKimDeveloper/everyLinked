...
const RecommendedUser = ({ user }) => {
    ...
    // SEND REQUESTS
    // 친구추가 보내기
    const { mutate: sendConnectionRequesst } = useMutation({
        mutationFn: (userId) => axiosInstance.post(`/connections/request/${userId}`),
        onSuccess: () => {
            toast.success("Connection requests sent successfully")
    ...
    // USEQUERY <--->  SEND THE REQUEST <----> API <-----> Response <----> USEQUERY <---->UI
    // 친구추가 승락하기
    ...
    const { mutate: rejectRequest } = useMutation({
        ...
        onError: (error) => {
            console.error("Error in rejectRequest Request", error)
            toast.error(error.response?.data?.error || "An error occurred")
        }
    })
    ...
    const handleConnect = () => {
        ...
        if (connectionStatus?.data?.status === "not_connected") {
            sendConnectionRequesst(user._id);
        }
    };
    console.log("status from API:", connectionStatus?.data?.status)
